...
void SceneLight::renderMetagross()
{
	modelStack.PushMatrix();
...
void SceneLight::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void SceneLight::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
	if(!mesh || mesh->textureID <= 0) //Proper error check
		return;
